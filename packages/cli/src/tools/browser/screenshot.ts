...
export async function runBrowserScreenshot(input: unknown, modelId?: string): Promise<(ImagePart | TextPart)[]> {
  ...
  if (modelId && isVisionModel(modelId)) {
    const buffer = await page.screenshot({ type: "jpeg", quality: 85, fullPage });
