so now i want to add the memory tool we discussed about and stated in the `context/progress-tracker.md`. where the model will be able to set, delete , update and list the memory. the api has already been implemented so its to wire it as a tool to the llm.

it's meant to be a tool right? no other approach
