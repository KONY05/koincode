i want to improve the system prompt we have in `packages/server/src/system-prompt.ts`.

i want to capitalize upon what we have and at the same time take inspiration from the source below, being able to read Agents.md, inserting the user memory (can be commented out till we implement it, might add a memory table to store user's memory, fetch it and insert it into the system prompt), get environment details (type of system it's working on, current date etc), guidelines to writing good code, i don't know if we should add the developer instructions, user instructions cause we don't have anywhere we are storing that information or handling it. let me know your thoughts on the questions proposed before we move ahead and let me know the things you want to keep as well as remove from adding to our own system prompt
