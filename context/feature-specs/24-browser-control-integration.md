...
## Follow-up: `serverStart` never surfaced logs, and nothing ever stopped the server

Found while working on `41-background-task-result-delivery.md`: `runServerStart` spawned with `stdout: "ignore", stderr: "ignore"` — a failed startup (missing dependency, syntax error, port already in use) returned only `"Timed out waiting for port N after Ns"`, with zero information about *why*, since the process's actual output was discarded rather than captured. Separately, once a server did start successfully, nothing ever stopped it — no registration with the cancel-on-unmount infra `13-subagent-tool-implementation.md`'s follow-up built for backgrounded shell commands, so a server started mid-session would survive `/exit`/Ctrl+C exactly like backgrounded shell commands did before that fix, and there was no explicit way for the agent to stop it either (no `serverStop` counterpart to `browserClose`).

Fixed with three changes:

- **Continuous log capture**: `stdout`/`stderr` switched from `"ignore"` to `"pipe"`. New `packages/cli/src/lib/background/server-log-buffer.ts` — a PID-keyed, tail-capped (20k chars) in-memory buffer, same process-lifetime-only shape as the other registries in `lib/background/`. Two background reader loops (`streamToLogBuffer`, one for stdout, one for stderr) start the moment the process spawns and keep appending for the process's entire lifetime — not just during the readiness-poll window — so logs are available for the whole time a server runs, not only its startup banner. On timeout, the captured buffer is included directly in the returned `logs` field, so the agent sees the actual failure (e.g. a stack trace) on the very call that failed, without a separate round trip.
- **On-demand log access while running**: new `checkServerLogs({ pid })` tool (`tools/browser/check-server-logs.ts`) — read-only query against the buffer, also reports whether the process is still running (via the existing `background-process-status.ts` registry, reused as-is). Deliberately a *pull* tool, not a push/auto-delivery like backgrounded shell's — a dev server has no "finished" event to hang a one-shot delivery off, it's meant to keep running, so the agent polls it whenever it wants (e.g. after triggering a request in the browser, check here for a server-side error the browser console wouldn't show).
- **Explicit + implicit stop**: new `serverStop({ pid })` tool (`tools/browser/server-stop.ts`) — looks up the PID in `background-process-status.ts`, sends `SIGTERM` directly via the global `process.kill(pid, "SIGTERM")` (the tool file has no handle to the original `Bun.Subprocess` object, only the PID, since it's a separate call from `serverStart`'s). For implicit cleanup, `runServerStart` now also calls `registerBackgroundWork(sessionId, () => proc.kill())`, the exact same registration backgrounded shell commands use — so a server left running also gets killed on session unmount, `/exit`, and Ctrl+C, closing the same gap that was already fixed for shell in `41-background-task-result-delivery.md`.

Both new tools live in `browserToolContracts` (same gating as `serverStart` itself — BUILD mode + browser tools opt-in flag) rather than `readOnlyToolContracts`, since they're only useful in combination with a `serverStart` call that's itself gated the same way. System prompt's `getBrowserControlSection()` (`system-prompt.ts`) updated: the workflow steps now mention `checkServerLogs` for server-side errors alongside the existing `browserGetConsoleLogs` for client-side ones, and step 6 ("always clean up") now includes `serverStop` alongside `browserClose`.

Not yet verified live — verified via `bun run typecheck`/lint and code review only.
